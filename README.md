...
- [~] **V1.1 — the 30-day soak's findings, fixed:** the first soak passed on cost and rumour lifecycle and *failed* the continuity exit with four contradictions, so every failure was root-caused against the event log and closed — scenes no longer see their own prior output (64% of every prompt block used to be the household's own words), ids arrive with names and events with dates, witnesses keep what they saw, attention rotates instead of freezing, work is counted by absence rather than by activity strings, and `scripts/audit_run.py` turns all of it into 27 mechanical probes. The re-soak's verdict: the **mechanical gate passes** (10 failing probes -> 0, exit 0) and the **narrative gate does not**. Days 0-11 of the followed family are clean; days 14-19 cluster on one mechanism the fixes missed — a memory written in relative time ("last night") stays in the digest and is re-read days later as if still true. Fixed mechanically after the run, not yet verified over 30 days. Full numbers in [runs/soak2/REPORT.md](runs/soak2/REPORT.md).
